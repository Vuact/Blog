...
但是如果我们调换顺序，先点击 `log` 按钮再点击 +1 按钮，就会发现页面上的 `n` 和控制台打印的 `n` 并不是一样的，控制台打印的结果并不是 +1 之后的结果，也就是说当我们执行 `log` 函数的时候，尽管在这个期间已经改变了 `n` 值，但是在三秒钟之后打印的 n 值却不是 +1 之后的结果，

由此可以说明 `每一次渲染都是一个“新版本”`（即：`在任意一次渲染中，props和state是始终保持不变的`）[本质是由闭包实现的]，这也正是 React 所认定的`“数据不可变”思想`。
