...
>你可能有疑问：闭包保存的对象是其引用地址，那为什么`showMessage`中调用`props.user`读取到的不是最新的值，而是一个较老的值呢？
>因为每次React组件渲染时，都会生成新的`props`对象，这个对象的引用地址不同于上一次的`props`对象。所以即使父组件传入新的`props`，在闭包中仍然能访问到创建闭包时的那个`props`对象。（当然如果父组件更新，但子组件没有重新渲染，那么子组件`props`也不会重新生成，故引用地址也一致不会变化）
