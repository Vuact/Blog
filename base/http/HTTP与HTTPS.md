...
Bob 先将消息 `用Hash函数生成消息摘要`，再用自己的`私钥`加密生成`数字签名`，然后再将这个签名附在消息下面，一起发给Alice。

接下来就是 Alice 校验数字签名的流程了。

> - 数字签名就是 将消息摘要用私钥加密后 的产物
> - 其实你可以把摘要算法理解成一种特殊的压缩算法，它能够把任意长度的数据压缩成一种固定长度的字符串。
...
Alice收到消息后，取下数字签名，再用Bob的公钥解密，得到消息的摘要。

Alice再对消息本身使用`Hash函数`产生一个`新的摘要信息`，再与上一步得到的`摘要进行对比`。如果两者一致，就证明这个消息未被修改过，因此数字签名能够验证信息的完整性。

> 公钥和私钥通常有可以互相加解密的特性：将原始信息用公钥加密后，可以使用私钥解密； 将原始信息用私钥加密后，通常可以使用公钥解密。

<br>

当然，上述过程的`前提是Alice知道Bob的公钥`。问题的关键和消息本身一样，`公钥不能在不安全的网络中直接发送`给Alice，或者说拿到的公钥如何证明是Bob的呢？

此时就需要引入了证书颁发机构（`Certificate Authority`，简称`CA`），`CA`数量并不多，Bob客户端内置了所有受信任`CA`的证书。`CA`对Bob的公钥（和其他信息）数字签名后生成证书。
...
HTTPS连接 共需要`7次握手`：即`3次TCP` + `4次TSL`。
...
- 第一次：客户端向服务端发送：客户端 支持的 TLS版本号、加密算法、压缩算法 等
- 第二次：服务端向客户端发送：应该使用的加密算法等 + 自己的公钥 + 自己的证书 等；
- 第三次：客户端向服务端发送：自己的证书 + 通过服务器要求的加密算法生成`密钥`（就是个随机值）用于对称加密 等等；
- 第四次：服务端向客户端发送：使用服务端的`私钥`解密出对称秘钥（随机值）后，发送加密的Finish消息，表明完成握手
...
![image](https://user-images.githubusercontent.com/74364990/175875161-df46d1d8-5b68-40f5-bbdc-2edcd349ab4d.png)
...
- HTTPS 协议`握手阶段`比较`费时`，会使页面的加载时间延长近 50%，增加 10% 到 20% 的耗电；
- HTTPS 连接缓存不如 HTTP 高效，会增加数据开销和功耗
- SSL 证书`需要钱`，功能越强大的证书费用越高，个人网站、小网站没有必要一般不会用。
