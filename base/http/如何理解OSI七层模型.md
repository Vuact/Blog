...
![image](https://user-images.githubusercontent.com/74364990/174145724-4c144377-315e-4a4f-af94-e7f50cbaa79a.png)


## 应用层
应用层位于 `OSI` 参考模型的第七层，其作用是通过应用程序间的交互来完成特定的网络应用

该层协议定义了应用进程之间的交互规则，通过不同的应用层协议为不同的网络应用提供服务。例如域名系统 `DNS`，支持万维网应用的 `HTTP` 协议，电子邮件系统采用的 `SMTP协议`等
...
该层提供的服务主要包括`数据压缩，数据加密以及数据描述`，使应用程序`不必担心在各台计算机中表示和存储的内部格式差异`
...
在两个相邻节点之间传送数据时，数据链路层将网络层交下来的 `IP`数据报`组装成帧`，在两个相邻节点间的链路上传送帧
...
![image](https://user-images.githubusercontent.com/74364990/174146158-84c2641f-9180-4e1f-99e8-717dbb4f5841.png)
