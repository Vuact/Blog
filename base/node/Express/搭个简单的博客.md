...
默认入口文件是index.js，我们将入口文件改为app.js，修改package.json文件：
...
当浏览器访问服务器并发送第一次请求时，服务器端会创建一个session对象，生成一个类似于key,value的键值对， 然后将key(存在cookie中)返回到浏览器(客户)端，浏览器下次再访问时，携带key(cookie)，去服务器找到对应的session(value) 。 客户的信息都保存在session中。
